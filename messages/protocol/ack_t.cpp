...
namespace raccoon
{
    int ack_t::encoded_size() const noexcept
    {
        return 8 + codec::size_of_string(publisher_id) + 8 + codec::size_of_string(subscriber_id);
    }

    int ack_t::encode(uint8_t* buf, int buf_len) const noexcept
    {
    ...
    int ack_t::decode(const uint8_t* buf, int buf_len) noexcept
    {
